 ...
 * 战斗结束后，通知agent服务战斗结果，释放战斗池服务的唯一key，取消agent服务到战斗服的注册，解除战斗服playerObj与agent玩家对象的绑定，并移除战斗服务中的playerObj
 ![battleProces](/src/ATB回合制战斗/1.jpg)
...
![targetSelect](/src/ATB回合制战斗/3.jpg)
...
```
（base + addbase1 + addbase2）* （1 + 百分比add1 + 百分比add2）
```
